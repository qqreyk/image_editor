...
TEST_CASE("applyColorFilter: выбрасывает исключение для 1-канального изображения") {
    Image img(2, 2, 1);
    CHECK_THROWS_AS(filters::applyColorFilter(img, 100, 100, 100), std::invalid_argument);
}

TEST_CASE("Image: конструктор с корректными параметрами создаёт изображение") {
    Image img(3, 5, 3);
    CHECK(img.width() == 3);
    CHECK(img.height() == 5);
...
TEST_CASE("Image: конструктор с нулевой шириной выбрасывает исключение") {
    CHECK_THROWS_AS(Image(0, 5, 3), std::invalid_argument);
}

TEST_CASE("Image: конструктор с отрицательной высотой выбрасывает исключение") {
    CHECK_THROWS_AS(Image(5, -1, 3), std::invalid_argument);
}

TEST_CASE("Image: конструктор с нулевым числом каналов выбрасывает исключение") {
    CHECK_THROWS_AS(Image(5, 5, 0), std::invalid_argument);
}

TEST_CASE("Image: загрузка несуществующего файла выбрасывает исключение") {
    CHECK_THROWS_AS(Image("несуществующий_файл_12345.png"), std::runtime_error);
}

TEST_CASE("Image::at: корректные координаты не выбрасывают исключение") {
    Image img(2, 2, 3);
    CHECK_NOTHROW(img.at(0, 0, 0));
    CHECK_NOTHROW(img.at(1, 1, 2));
}

TEST_CASE("Image::at: отрицательная координата выбрасывает исключение") {
    Image img(2, 2, 3);
    CHECK_THROWS_AS(img.at(-1, 0, 0), std::out_of_range);
}

TEST_CASE("Image::at: координата за пределами ширины выбрасывает исключение") {
    Image img(2, 2, 3);
    CHECK_THROWS_AS(img.at(2, 0, 0), std::out_of_range);
}

TEST_CASE("Image::at: номер канала за пределами диапазона выбрасывает исключение") {
    Image img(2, 2, 3);
    CHECK_THROWS_AS(img.at(0, 0, 3), std::out_of_range);
}
